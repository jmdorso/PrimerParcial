...
int procesarResiduosPorID(sPedido *aArray,int cantidad,int id)
{
	...
	if(aArray != NULL && cantidad>0 && id>=0)
	{
		...
		if(posArray != -1)
		{
			...
			printf("\n\nClasificacion de residuos: Ingrese KG de cada tipo (no sobrepasar el total [%.2f Kg]",auxPedido.cantKg);
			getValidFloatFromString(&bufferHDPE,"\nIngrese cant. KG de residuos HDPE: \n",
					"\nError!",0,CANT_MAX_KG,CANT_REINTENTOS);
			getValidFloatFromString(&bufferLDPE,"\nIngrese cant. KG de residuos LDPE: \n",
								"\nError!",0,CANT_MAX_KG,CANT_REINTENTOS);
			getValidFloatFromString(&bufferPP,"\nIngrese cant. KG de residuos PP: \n",
								"\nError!",0,CANT_MAX_KG,CANT_REINTENTOS);
