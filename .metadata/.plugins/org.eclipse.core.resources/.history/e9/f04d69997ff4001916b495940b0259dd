...
int altaUnSoloPedidoPorUI(sPedido *pedido,sCliente *aCliente,int cantCliente)
{
	...
	if(retorno == EXIT_SUCCESS)
	{
		...
		{
			printf("\nError en el ID. Regresara al menu principal!\n");
			retorno=EXIT_ERROR;
		}
...
/** \brief cuenta e imprime los pedidos en estado pendiente de cada cliente
 * \param *aArray es el array de clientes
 * \param cantidad es la cantidad de elementos del array de clientes
 * \param *aPedido es el array de Pedidos
 * \param cantPedido es la cantidad de elementos del array de pedidos
 * \param *aContCliente es el array auxiliar de contadores de pedidos de cada cliente
 * \param cantContCliente es la cantidad de elementos del array auxiliar
 * \return -1 si hay algun error o 0 si esta bien
 *
 */
...
/** \brief busca un cliente por ID y lo da de baja mediante el campo status (dejandolo EMPTY) y si posee pedidos tambien los dara de baja
 * \param *aArray es el array de clientes
 * \param cantidad es la cantidad de elementos del array
 * \param id es el ID a buscar
 * \param *aPedidos el array de pedidos
 * \param cantPedidos la cantidad de elementos del array de pedidos
 * \return -1 si hay algun error o 0 si esta bien.
 *
 */
		...
		if(posArray != -1)
		{
			...
			for(i=0;i<cantPedidos;i++)
			{
				if(aPedidos[i].idCliente==id)
				{
					...
					aPedidos[i].status=STATUS_EMPTY;
				}
			}
			retorno = EXIT_SUCCESS;
		}
